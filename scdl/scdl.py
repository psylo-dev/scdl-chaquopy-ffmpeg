...
from ffmpeg_handler import FFmpegHandler, FFmpegError
...
class SafeLock:
    def __init__(
        ...
        self._soft_lock = SoftFileLock(lock_file, timeout=timeout)
...
def re_encode_to_buffer(
    ...
    ffmpeg_handler = FFmpegHandler(debug=kwargs.get("debug", False), hide_progress=kwargs.get("hide_progress", False))

    if skip_re_encoding and isinstance(in_data, requests.Response):
        encoded_data = ffmpeg_handler.copy_stream(in_data)
    else:
        encoded_data = ffmpeg_handler.re_encode(
            in_data,
            out_codec,
            should_copy,
            track.duration,
